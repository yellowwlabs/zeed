# Dashboard IPO List + Banner API Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign `/dashboard` to match the web3-dashboard dark UI, show companies as IPO listings from the DB, and add a backend banner API (admin-only create/update/delete) powering the hero carousel.

**Architecture:** Add a `Banner` model and `role` field to the Prisma schema, expose a `bannerRouter` via tRPC (list = any auth, mutations = admin-only via new `adminProcedure`), then rebuild `/dashboard/page.tsx` to mirror the web3-dashboard layout pulling real company + banner data.

**Tech Stack:** Next.js 14 (App Router), tRPC, Prisma (PostgreSQL), Zod, Tailwind CSS, lucide-react
...
| Action | Path | Purpose |
|--------|------|---------|
| Modify | `backend/prisma/schema.prisma` | Add `role` to User, add `Banner` model |
| Modify | `backend/src/middlewares/role.middleware.ts` | Add `adminProcedure` |
| Create | `backend/src/routes/v1/banner.ts` | Banner tRPC router |
| Modify | `backend/src/routes/index.ts` | Register bannerRouter |
| Modify | `frontend/app/dashboard/page.tsx` | Full UI redesign |

---

### Task 1: Add `role` to User model and `Banner` model in Prisma schema
...
In `schema.prisma`, inside the `User` model after `image String?`, add:
...
- [ ] **Step 2: Add Banner model**

Append at the end of `schema.prisma` (after the FounderMajorityProof model):
...
Also add the reverse relation to the User model — inside `User`, add:
...
Expected: "Your database is now in sync with your Prisma schema" and client regenerated.

- [ ] **Step 4: Commit**

```bash
git add backend/prisma/schema.prisma
git commit -m "feat(schema): add Banner model and User.role field"
```

---

### Task 2: Add `adminProcedure` to role middleware

**Files:**
- Modify: `backend/src/middlewares/role.middleware.ts`

- [ ] **Step 1: Add adminProcedure export**

Append to `backend/src/middlewares/role.middleware.ts`:
...
Expected: no errors related to role.middleware.ts.

- [ ] **Step 3: Commit**

```bash
git add backend/src/middlewares/role.middleware.ts
git commit -m "feat(auth): add adminProcedure middleware"
```

---

### Task 3: Create banner tRPC router

**Files:**
- Create: `backend/src/routes/v1/banner.ts`
...
In `backend/src/routes/index.ts`, add the import and register:
...
```bash
git add backend/src/routes/v1/banner.ts backend/src/routes/index.ts
git commit -m "feat(api): add banner tRPC router with admin-gated mutations"
```

---

### Task 4: Redesign `/dashboard/page.tsx` with web3-dashboard UI + real data

**Files:**
- Modify: `frontend/app/dashboard/page.tsx`

This task replaces the entire file. The new page mirrors the web3-dashboard layout exactly (same nav, left sidebar, right sidebar, bottom bar) but pulls real data: banners from `trpc.banner.list` for the hero carousel, companies from `trpc.company.list` for the IPO grid.
...
Expected: no type errors in `app/dashboard/page.tsx`.

- [ ] **Step 3: Commit**

```bash
git add frontend/app/dashboard/page.tsx
git commit -m "feat(dashboard): redesign with web3 UI, IPO company grid, and banner carousel"
```
...
- [x] **Spec coverage:** Banner CRUD (admin) ✓ | Banner list (auth) ✓ | Company IPO grid from DB ✓ | web3-dashboard UI match ✓ | Dynamic carousel ✓
- [x] **Placeholders:** None — all code blocks are complete
- [x] **Type consistency:** `banner.list` returns `Banner[]` used in the component; `company.list` returns with `_count.deals` consistent with existing `CompanyController.list` pattern; `adminProcedure` exported from middleware and imported in banner router
